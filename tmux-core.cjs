...
function resolveCurrentPaneContext({ envPaneId, displayPaneId, ctxCwd, displayCwd } = {}) {
  return {
    paneId: String(displayPaneId || envPaneId || '').trim(),
    cwd: String(displayCwd || ctxCwd || '').trim(),
  };
}
